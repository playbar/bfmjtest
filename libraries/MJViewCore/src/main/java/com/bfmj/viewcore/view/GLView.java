...
public abstract class GLView implements GLRenderListener {
	...
	private boolean isHandleFocus = true;
	
	public GLView(Context context){
		this(context, null);
	}

	public GLView(Context context, GLRootView rootView){
		mContext = context;
		mMatrixState = new GLMatrixState();

		if (rootView == null && getContext() instanceof BaseViewActivity){
			BaseViewActivity activity = (BaseViewActivity)getContext();
			if (activity != null) {
				rootView = activity.getRootView();
			}
		}
		mRootView = rootView;
	}
	...
	public GLRootView getRootView(){
		if (mRootView == null && getContext() instanceof BaseViewActivity){
			BaseViewActivity activity = (BaseViewActivity)getContext();
			if (activity != null) {
				mRootView = activity.getRootView();
			}
		}
		return mRootView;
	}
	...
	public void setEyeDeviation(float mEyeDeviation) {
		...
		if (this instanceof GLGroupView){
			...
			for (int i = 0; i < views.size(); i++){
				views.get(i).setEyeDeviation(mEyeDeviation);
			}
		}
	}
