...
//	private GLRectView convertView;
	...
	protected ScrollDirection mScrollDirection = ScrollDirection.LEFT_RIGHT;
	...
	public void showHItem( int cIndex ){
		...
		for(int rows=0; rows<this.mNumRows; rows++)
		{
			for(int col=0;col<this.mNumColumns;col++)
			{
				...
				float fx = getX() + getPaddingLeft() + view.getWidth() * col + this.mHorizontalSpacing * col;
...
//				float fx = getX() + getPaddingLeft() + getMarginLeft() + view.getWidth() * col + this.mHorizontalSpacing * col;
				float fx = (getX() + getWidth()) - getPaddingLeft() - (view.getWidth() + this.mHorizontalSpacing) * (mNumColumns - col);
				if( EMoveDirection.MOVERTOL == mMoveDirection){
					fx += this.getWidth();
				}else if( EMoveDirection.MOVELTOR == mMoveDirection){
					fx -= this.getWidth();
...
//				view.setX((getX() + getWidth()) - getPaddingLeft() - getMarginRight() - (view.getWidth() + this.mHorizontalSpacing) * (mNumColumns - col) );
	...
	public void setTranslateAnimation(GLRectView view, float x, float y,float z){
		if(view == null)
			return;
		GLAnimation animation = new GLTranslateAnimation(x, y, z);
		animation.setAnimView(view);
		animation.setDuration(500);
		animation.setOnGLAnimationListener(new GLAnimation.OnGLAnimationListener() {
			@Override
	...
	public void pageChange(){
		...
		if (ScrollDirection.UP_DOWN == mScrollDirection){
			if(EMoveDirection.MOVERTOL == mMoveDirection ){
				fx = -this.getHeight();
			}else if( EMoveDirection.MOVELTOR == mMoveDirection){
				fx = this.getHeight();
		...
		for (int i = 0; i < count; ++i){
			if(ScrollDirection.UP_DOWN == mScrollDirection){
				setTranslateAnimation(mItemViewBack.get(i), 0, fx, 0);
			}else {
		...
		for (int i = 0; i < count; ++i){
			if( ScrollDirection.UP_DOWN == mScrollDirection){
				mItemViewCur.get(i).setX(getX() + (i % mNumColumns) * ((getWidth() - mHorizontalSpacing * (mNumColumns - 1)) / mNumColumns + mHorizontalSpacing));
				mItemViewCur.get(i).setY(getY() + (i / mNumColumns) * ((getHeight() - mVerticalSpacing * (mNumRows - 1)) / mNumRows + mVerticalSpacing) - fx);
				setTranslateAnimation(mItemViewCur.get(i), 0, fx, 0);
			}else {
