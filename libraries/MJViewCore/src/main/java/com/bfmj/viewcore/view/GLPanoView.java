...
public class GLPanoView extends GLView {
    ...
    private boolean isObjFileLoaded = false;

    private float[] mRotateHeadview = new float[3];

    private static GLPanoView instance = null;

    private TextureLoader.TextureType mLeftTextureType = TextureLoader.TextureType.TEXTURE_TYPE_BITMAP;
    private TextureLoader.TextureType mTextureType = TextureLoader.TextureType.TEXTURE_TYPE_BITMAP;
    ...
    public GLPanoView(Context context, GLRootView rootView) {
        super(context, rootView);
        new Thread(new Runnable() {
            @Override
            public void run() {
                loadObj();
                isObjFileLoaded = true;
            }
        }).start();
    }
     ...
     * @return
     */
    public void setImage(int resId, TextureLoader.TextureType textureType){
        if (mResId == resId){
            return;
        }
    ...
    public void setLeftImage(int resId, TextureLoader.TextureType textureType){
        if (mLeftResId == resId){
            return;
        }

        mLeftTextureType = textureType;
    ...
    public void onBeforeDraw(boolean isLeft) {
        if (!isObjFileLoaded || !isVisible() || !isSurfaceCreated || vertices[mSceneType] == null || mTextureId < 0){
            return;
        }
    ...
    public void createTexture(){
        ...
        /// right
        if (mResId > -1){
            if (mTextureType == TextureLoader.TextureType.TEXTURE_TYPE_BITMAP){
                ...
                try {
                    ...
                    TextureLoader.LoadTextureFromResource(getContext().getResources(), mResId, tex, mTextureType);
                    mTextureId = tex.textureID;
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        bitmap = null;
        if (mLeftResId > -1){
            if (mLeftTextureType == TextureLoader.TextureType.TEXTURE_TYPE_BITMAP){
                ...
                try {
                    ...
                    TextureLoader.LoadTextureFromResource(getContext().getResources(), mLeftResId, tex, mLeftTextureType);
                    mLeftTextureId = tex.textureID;
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            ...
            if (getRootView() != null) {
                getRootView().queueEvent(new Runnable() {
                    ...
                    public void run() {
                        ...
                        GLES30.glDeleteBuffers(vboVertexBufferIds.length, vboVertexBufferIds, 0);
                        for (int i = 0; i < vboTexCoorBufferIds.length; i++){
                            GLES30.glDeleteBuffers(vboTexCoorBufferIds[i].length, vboTexCoorBufferIds[i], 0);
                        }
                    }
                });
            }
