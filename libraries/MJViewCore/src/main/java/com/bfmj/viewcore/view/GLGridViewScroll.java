...
//	}
	...
	public void nextPage(){
		if( mCurIndex < mCount && mbMoveEnd ){
			...
			setStartIndex((mCurIndex - 1) * getNumOneScreen());
			pageChange();
			mProcessView.setProcessAnimation( mCount, mScrollDirection);
		}
	}
...
//		}
	...
	public void resetPage(){
		...
		if (mScrollDirection == ScrollDirection.UP_DOWN){
			mProcessView.setBarLayoutParams((int)mProcessViewWidth,
					mCount > 0 ?  (int)(mProcessViewHeight / mCount) : (int)mProcessViewHeight);
			mProcessView.setX(getX() + getWidth() + mBtnSpace);
			mProcessView.setY(mOffsetY + getY() + (getHeight() - mProcessViewHeight) / 2);
		} else {
			mProcessView.setBarLayoutParams(mCount > 0 ? (int)(mProcessViewWidth / mCount) : (int)mProcessViewWidth,
					(int)mProcessViewHeight);
...
//							requestLayout();
	...
	private void showPrvBtn(){
		if (mScrollDirection == ScrollDirection.UP_DOWN){
			mPrvBtnImgView.setX(getX() +getWidth()  + mBtnSpace - (mBtnImageWidth - mProcessViewWidth)/2);
			mPrvBtnImgView.setY(mOffsetY + getY() + (getHeight() - mProcessViewHeight) / 2 - mBtnImageHeight - mBtnHorSpace);
		} else {
...
//		});
	...
	private void showNextBtn(){
		if (mScrollDirection == ScrollDirection.UP_DOWN) {
			mNextBtnImgView.setX(getX() + getWidth() + mBtnSpace - (mBtnImageWidth - mProcessViewWidth) / 2);
			mNextBtnImgView.setY(mOffsetY + getY() + (getHeight() + mProcessViewHeight) / 2 + mBtnHorSpace);
		} else {
...
//		}
	...
	protected void scale(float sx, float sy, float[] centerPos) {
		...
		mOffsetX *= sx;
		mOffsetY *= sy;
		mProcessViewWidth *= sx;
		mProcessViewHeight *= sy;
