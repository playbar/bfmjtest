...
public class AsyncHttpClient {
    ...
    protected static final Map<Context, List<RequestHandle>> requestMap = Collections.synchronizedMap(new WeakHashMap<Context, List<RequestHandle>>());
     ...
     *                              pending requests.
     */
    public static void cancelRequests(final Context context, final boolean mayInterruptIfRunning) {
        if (context == null) {
    ...
    public RequestHandle post(Context context, String url, HttpEntity entity, String contentType, ResponseHandlerInterface responseHandler) {
        try {
            if (!(entity instanceof SimpleMultipartEntity)) {
                LogMessage.i("post-url: ", url + "?" + EntityUtils.toString(entity));
            } else {
                LogMessage.i("post-url: ", url + "\nparames: \n包含文件流");
            }
        } catch (Throwable e) {
            e.printStackTrace();
        }
