...
import com.baofeng.mj.bean.Resource.NotifyResourceChange;
import com.baofeng.mj.business.localbusiness.flyscreen.logic.FlyScreenLoginModel;
import com.baofeng.mj.business.localbusiness.flyscreen.logic.FlyScreenResListModel;
...
public class FlyScreenInterface implements TcpConnCallback {
	...
	private static FlyResType resType = FlyResType.video;
	...
	public void getDeviceResourceList(int index, boolean fresh) {
		boolean connect = NetworkUtils.getNetStatus(mContext);
		if (!connect) {
...
//		}
	...
	private void requestLoginData(DeviceInfo currDevInfo) {
		...
		tcpClient.tcpConnectAndCallback(currDevInfo.getIp(), currDevInfo.getPort(), this);
			...
			public void handlerMessage(Message msg) {
				...
				switch (msg.what) {
					case MSG_TCP_DISCONNECTED:
						...
						Toast.makeText(mContext,"您的电脑已经关闭飞屏功能，页面即将调转 ?", 1500);
						break;
					...
					case MSG_UPDATE_RES_DIR_PAGE:
						...
						switch (responsePageData.getErrorcode()) {
							case ResourceErrorCode_OK:
								// 判断返回的数据是否是当前进入的目录
								if (Common.valid(responsePageData.getUri(), true)
										&& !getCurDirUri().equals(
...
//								}
					...
					case MSG_UPDATE_RES_NOTIFY_CHANGE:
						NotifyResourceChange notifyResourceChange = null;
						try {
	...
	private void getDirCount(String dir) {
		...
		tcpClient.send(resListModel.createtDirCountRequest(resType, dir));
	}
