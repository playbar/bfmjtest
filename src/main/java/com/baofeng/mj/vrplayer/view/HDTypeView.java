...
//            linearView.scale(GLConst.Player_Settings_Scale);
            for(int x = 0; x < strs.length; x++) {
                ...
                glTextView.setBackground(R.drawable.play_bg_control_normal_150_60);
                ...
                if(strs[x].endsWith("k")){
                    glTextView.setText(strs[x].substring(0,strs.length-1)+"K");
                }else if(strs[x].endsWith("p")){
                    glTextView.setText(strs[x].substring(0,strs.length-1)+"P");
                            ...
                            if(view instanceof GLTextView) {
                                if(!((GLTextView)view).isSelected()) {
                                    if(focused) {
                                        ...
                                        ((GLTextView)view).setBackground(R.drawable.play_bg_control_hover_150_60);
                                    } else {
                                        ((GLTextView)view).setTextColor(new GLColor(0x888888));
                                        ((GLTextView)view).setBackground(R.drawable.play_bg_control_normal_150_60);
                                    }
                                }
                            }
                        }
    ...
    private void clearSelected() {
        for(GLRectView childView : linearView.getChildView()) {
            if(childView instanceof GLTextView) {
                ...
                ((GLTextView) childView).setBackground(R.drawable.play_bg_control_normal_150_60);
            }
        }
    }
