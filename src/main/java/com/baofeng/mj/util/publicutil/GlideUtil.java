...
//            builder = requestManager.load(drawable).centerCrop().placeholder(defaultDrawable).skipMemoryCache(true).crossFade().dontAnimate().diskCacheStrategy(DiskCacheStrategy.RESULT);
        }else if(url.endsWith(".gif")){//gif动态图（磁盘缓存策略用DiskCacheStrategy.SOURCE，不然加载会很慢）
    ...
    private static void displayImageSplash(Context context, WeakReference<ImageView> weakIv, String url, int defaultDrawable, int width, int height){
        ...
        if(url.endsWith(".gif")){//gif动态图（磁盘缓存策略用DiskCacheStrategy.SOURCE，不然加载会很慢）
            builder = requestManager.load(url).centerCrop().placeholder(defaultDrawable).skipMemoryCache(true).crossFade().diskCacheStrategy(DiskCacheStrategy.SOURCE);
        }else{//普通图
